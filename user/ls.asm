...
ls(char *path)
{
  ...
  switch(st.type){
  ca:	8b b5 c4 fd ff ff    	mov    -0x23c(%ebp),%esi
  d0:	66 83 fe 01          	cmp    $0x1,%si
  d4:	0f 84 90 00 00 00    	je     16a <ls+0xdf>
  da:	66 83 fe 02          	cmp    $0x2,%si
  de:	75 44                	jne    124 <ls+0x99>
  case T_FILE:
    printf(1, "%s %d %d %d\n", fmtname(path), st.type, st.ino, st.size);
  e0:	8b 85 d4 fd ff ff    	mov    -0x22c(%ebp),%eax
  e6:	89 85 b4 fd ff ff    	mov    %eax,-0x24c(%ebp)
  ec:	8b 8d cc fd ff ff    	mov    -0x234(%ebp),%ecx
  f2:	89 8d b0 fd ff ff    	mov    %ecx,-0x250(%ebp)
  f8:	83 ec 0c             	sub    $0xc,%esp
  fb:	53                   	push   %ebx
  fc:	e8 ff fe ff ff       	call   0 <fmtname>
 101:	83 c4 08             	add    $0x8,%esp
 104:	ff b5 b4 fd ff ff    	push   -0x24c(%ebp)
 10a:	ff b5 b0 fd ff ff    	push   -0x250(%ebp)
 110:	0f bf f6             	movswl %si,%esi
 113:	56                   	push   %esi
 114:	50                   	push   %eax
...
strlen(const char *s)
{
 ...
 338:	40                   	inc    %eax
 339:	80 3c 02 00          	cmpb   $0x0,(%edx,%eax,1)
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 586:	39 f2                	cmp    %esi,%edx
 588:	73 e2                	jae    56c <printint+0x26>
  if(neg)
